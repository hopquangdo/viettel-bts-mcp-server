...
public class McpToolDefinitions {
    ...
    @Tool(
        ...
        public SanLuongQueryResponse sanLuong(
            ...
            @ToolParam(description = "Khu vực cần lọc: ID, mã hoặc tên (ưu tiên truyền mã hoặc ID; tên mơ hồ sẽ báo lỗi kèm danh sách ứng viên)", required = false) String khuVuc,
            @ToolParam(description = "Tỉnh/thành cần lọc: ID, mã hoặc tên", required = false) String tinhThanh,
    ...
    public HopDongQueryResponse hopDong(
            @ToolParam(description = "Đối tượng cần lọc: ID, mã hoặc tên (ưu tiên truyền mã hoặc ID; tên mơ hồ sẽ báo lỗi kèm danh sách ứng viên)", required = false) String doiTuong,
            @ToolParam(description = "Hợp đồng cần lọc: ID, mã hoặc tên (ưu tiên truyền mã hoặc ID; tên mơ hồ sẽ báo lỗi kèm danh sách ứng viên)", required = false) String hopDong,
            @ToolParam(description = "Nhà thầu cần lọc: ID, mã hoặc tên (ưu tiên truyền mã hoặc ID; tên mơ hồ sẽ báo lỗi kèm danh sách ứng viên)", required = false) String nhaThau,
            @ToolParam(description = "Khu vực cần lọc: ID, mã hoặc tên (ưu tiên truyền mã hoặc ID; tên mơ hồ sẽ báo lỗi kèm danh sách ứng viên)", required = false) String khuVuc,
        ...
        return hopDongToolHandler.query(doiTuong, hopDong, nhaThau, khuVuc, tinhThanh, loaiHopDong, kieuHopDong, query, page, pageSize, nguongChamTienDo, nguongXanh, nguongVang);
    }
    ...
    public TramTonQueryResponse doiTuongTon(
            ...
            @ToolParam(description = "Ngưỡng số ngày cho thieuCapNhat (đối tượng không cập nhật quá số ngày này), mặc định 7; nếu bỏ trống có thể dùng sinceDate", required = false) Integer soNgayThieuCapNhat) {
                return doiTuongTonToolHandler.query(doiTuong, hopDong, nhaThau, khuVuc, tinhThanh, loaiHopDong, tab, sinceDate, quaHanNgay, top, page, pageSize, soNgayThieuCapNhat);
    }
    ...
    public VuongMacQueryResponse vuongMac(
            ...
            @ToolParam(description = "Ngưỡng số ngày để coi vướng mắc là quá hạn cho khối quaHan, mặc định 30 (nếu bỏ trống có thể dùng sinceDate)", required = false) Integer quaHanNgay) {
                return vuongMacToolHandler.query(doiTuong, hopDong, nhaThau, khuVuc, tinhThanh, loaiHopDong, trangThai, kieuVuongMac, giaiDoan, dangMoOnly, query, top, sinceDate, page, pageSize, quaHanNgay);
    }
    ...
    public PhanCongQueryResponse phanCong(
            ...
            @ToolParam(description = "true: danhSach gồm cả phân công đã ngừng (lịch sử phân công của đối tượng), mặc định chỉ phân công đang hoạt động", required = false) Boolean lichSu,
                        ...
                        @ToolParam(description = "Số dòng mỗi trang, mặc định 3, tối đa 100", required = false) Integer pageSize) {
                return phanCongToolHandler.query(vung, nhaThau, canBo, query, hopDong, khuVuc, tinhThanh, doiTuong, giaiDoan, lichSu, top, page, pageSize);
    }
    ...
    public NguonViecQueryResponse nguonViec(
            @ToolParam(description = "Khu vực cần lọc: ID, mã hoặc tên (ví dụ TTKV2 nếu là mã trung tâm)", required = false) String khuVuc,
            @ToolParam(description = "Nhà thầu cần lọc: ID, mã hoặc tên (ưu tiên truyền mã hoặc ID; tên mơ hồ sẽ báo lỗi kèm danh sách ứng viên)", required = false) String nhaThau,
    ...
    public HoSoDoiTuongQueryResponse hoSoDoiTuong(
            ...
            @ToolParam(description = "true: chỉ đối tượng thuộc nhóm ưu tiên; false: chỉ đối tượng không thuộc nhóm ưu tiên; bỏ trống: tất cả", required = false) Boolean coNhomUuTien,
                        @ToolParam(description = "Số trang, bắt đầu từ 0", required = false) Integer page,
                        @ToolParam(description = "Số dòng mỗi trang, mặc định 3, tối đa 100", required = false) Integer pageSize) {
                return hoSoDoiTuongToolHandler.query(doiTuong, hopDong, nhaThau, khuVuc, tinhThanh, trangThaiHopDong, coNhomUuTien, page, pageSize);
    }
    ...
    public NganHoQueryResponse nganHo(
        ...
        return nganHoToolHandler.query(hopDong, query, heSo, nguongCanhBao, loaiHopDong, statusFilter, page, pageSize);
    }
    ...
    public SystemOverviewResponse systemOverview(
            @ToolParam(required = false) LocalDate fromDate,
            @ToolParam(required = false) LocalDate toDate) {
        return SystemOverviewResponse.builder()
                .hopDong(hopDongToolHandler.query(null, null, null, null, null, null, null, null, null, null, null, null, null))
                .sanLuong(sanLuongToolHandler.query(null, null, null, null, null, fromDate, toDate, null, null, null, null, null, null, null, null))
